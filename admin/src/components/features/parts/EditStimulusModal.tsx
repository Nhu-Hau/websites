...
interface EditStimulusModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onUpdate: (updatedStimulus: AdminStimulus) => void;
}
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200 border border-zinc-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100 bg-white sticky top-0 z-10">
          <div>
            <h2 className="text-xl font-bold text-zinc-900">Chỉnh sửa Stimulus</h2>
            <p className="text-sm text-zinc-500 font-mono mt-0.5">{stimulus.id}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          <form id="edit-stimulus-form" onSubmit={handleSubmit} className="space-y-8">
            {/* Media Section */}
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-zinc-100 bg-zinc-50/50">
          ...
          >
            <span>{uploading.uploading ? 'Đang xử lý...' : 'Lưu thay đổi'}</span>
          </button>
        </div>
      </div>
    </div>
