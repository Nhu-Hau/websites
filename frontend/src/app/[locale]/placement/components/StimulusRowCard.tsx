    ...
    <div id={`q-${displayIndex}`} className="rounded-xl border p-3">
      <div className="flex items-center justify-between mb-2">
        <div className="font-medium">
          Câu {displayIndex}:
          {/* <span className="text-xs text-gray-500">({item.part})</span> */}
        </div>
        {!locked ? (
          picked ? (
...
export function StimulusRowCard({
  ...
  onPick,
  showStimulusDetails,
  showPerItemExplain, // nếu muốn có explain theo từng câu, có thể thêm dưới ChoiceRow
}: {
  groupKey: string;
  ...
  return (
    <section className="rounded-2xl border-[2px] border-gray-300 p-4">
      <div className="grid grid-cols-3 gap-4">
        {/* LEFT: MEDIA CHUNG (y như layout cũ, chỉ render 1 lần) */}
        ...
        {/* RIGHT: TẤT CẢ CÂU CON (chỉ có stem nhỏ + choices) */}
