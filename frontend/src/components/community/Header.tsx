...
import { FaHome, FaUsers, FaBell, FaPlus } from "react-icons/fa";

type Props = {
  locale: string;
  active?: "community" | "home" | "notifications";
};

export default function Header({ locale, active = "community" }: Props) {
  const base = `/${locale}`;
  return (
    <header className="fixed left-0 right-0 z-40 bg-zinc-50 dark:bg-sky-900/30 shadow-sm border-b border-sky-200 dark:border-sky-800 top-16">
      <div className="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8 flex items-center justify-end">
        {/* Navigation */}
