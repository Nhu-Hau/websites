...
export default function GroupsClient() {
  ...
  const handleDelete = React.useCallback(
    async (groupId: string) => {
      if (!window.confirm("Bạn có chắc muốn xoá nhóm này?")) return;
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filteredGroups.map((group) => {
            ...
            return (
              <div
                key={group._id}
                className="group relative flex flex-col rounded-2xl border border-zinc-200/80 bg-white/95 p-4 shadow-sm ring-1 ring-black/[0.02] transition-all duration-150 hover:-translate-y-0.5 hover:border-sky-300 hover:shadow-md dark:border-zinc-800/80 dark:bg-zinc-900/95"
              >
                {isOwner && (
                  <button
                    type="button"
                    ...
                    className="absolute right-3 top-3 inline-flex items-center gap-1 rounded-full border border-red-200 bg-white/70 px-3 py-1 text-xs font-semibold text-red-600 shadow-sm transition hover:bg-red-50 disabled:opacity-50 dark:border-red-900/50 dark:bg-red-900/20 dark:text-red-300"
                  >
