...
export default function ActivityHeatmapClient({
      ...
      <div className="relative overflow-hidden rounded-2xl border border-zinc-200/80 bg-white/92 p-5 shadow-sm ring-1 ring-black/[0.03] transition-all duration-200 hover:shadow-md dark:border-zinc-800/80 dark:bg-zinc-900/92">
        ...
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-zinc-200 bg-zinc-50/80 px-4 py-7 text-center dark:border-zinc-700 dark:bg-zinc-900">
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400">
            Hãy bắt đầu làm bài luyện tập hoặc bài test để hệ thống vẽ heatmap
            cho bạn.
          </p>
        </div>
      </div>
  ...
  const getColor = (count: number) => {
    ...
    if (intensity < 0.25)
      return "bg-emerald-100/90 dark:bg-emerald-950/50 border border-emerald-100/70 dark:border-emerald-900/70";
    if (intensity < 0.5)
      return "bg-emerald-200/90 dark:bg-emerald-900/60 border border-emerald-200/80 dark:border-emerald-800/80";
    if (intensity < 0.75)
      return "bg-emerald-300/95 dark:bg-emerald-800/70 border border-emerald-300/90 dark:border-emerald-700/90";
    return "bg-emerald-400/95 dark:bg-emerald-700/80 border border-emerald-400/95 dark:border-emerald-600/90";
  };
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-zinc-200/80 bg-white/92 p-5 shadow-sm ring-1 ring-black/[0.03] transition-all duration-200 hover:shadow-md dark:border-zinc-800/80 dark:bg-zinc-900/92">
              ...
              <div key={wi} className="flex flex-col gap-[3px]">
                {week.map((day, di) => {
                  ...
                  return (
                    <div
                      key={di}
                      className={`h-3.5 w-3.5 rounded-[5px] transition-all duration-150 hover:scale-105 hover:ring-1 hover:ring-emerald-400/80 ${getColor(
                        day.count
                      )}`}
      ...
      <div className="flex flex-col gap-4 border-t border-zinc-200/80 pt-4 text-xs sm:flex-row sm:items-center sm:justify-between dark:border-zinc-800/80">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <div className="flex h-8 w-8 items-center justify-center rounded-full bg-amber-50 dark:bg-amber-900/25">
        ...
        <div className="flex items-center gap-2 text-[11px] text-zinc-500 dark:text-zinc-400">
          ...
          <div className="flex gap-1">
            ...
            <div className="h-3.5 w-3.5 rounded-[5px] bg-emerald-100/90 dark:bg-emerald-950/50" />
            <div className="h-3.5 w-3.5 rounded-[5px] bg-emerald-200/90 dark:bg-emerald-900/60" />
            <div className="h-3.5 w-3.5 rounded-[5px] bg-emerald-300/95 dark:bg-emerald-800/70" />
            <div className="h-3.5 w-3.5 rounded-[5px] bg-emerald-400/95 dark:bg-emerald-700/80" />
          </div>
          <span>Nhiều</span>
        </div>
      </div>
      ...
      {stats.currentStreak > 0 && (
        <div className="mt-3 rounded-xl border border-amber-200/80 bg-amber-50/80 px-4 py-2.5 text-center text-[11px] font-medium text-amber-800 dark:border-amber-800/80 dark:bg-amber-900/20 dark:text-amber-200">
          Bạn đang duy trì chuỗi học{" "}
          <span className="font-semibold">{stats.currentStreak}</span> ngày liên
          tiếp. Đừng để bị đứt nhé!
        </div>
      )}
    </div>
  );
