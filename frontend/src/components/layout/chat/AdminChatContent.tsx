...
export default function AdminChatContent({
  ...
  useEffect(() => {
    ...
    const onAdminMessage = (data: any) => {
      ...
      if (!open) {
        setUnreadCount((prev) => ({ ...prev, admin: prev.admin + 1 }));
      }
    };
  ...
  }, [socket, user, sessionId, open, setUnreadCount]);
  ...
  useEffect(() => {
    if (open) {
      setUnreadCount((prev) => ({ ...prev, admin: 0 }));
    }
  }, [open, setUnreadCount]);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {messages.length > 0 && user && user.access === "premium" && (
        <div className="flex-shrink-0 px-3 sm:px-4 pt-3 pb-2 flex justify-end">
          <button
            onClick={clearChat}
      ...
      <div
        ref={listRef}
        className={`${contentHeight} overflow-y-auto px-3 sm:px-4 py-4 space-y-4 bg-gradient-to-b from-transparent to-orange-50/20 dark:from-transparent dark:to-zinc-900/50`}
      >
        {messages.length === 0 ? (
      ...
      <div className="border-t border-gray-200/60 dark:border-zinc-700/60 p-3 sm:p-4 bg-white/70 dark:bg-zinc-900/70 flex-shrink-0">
          ...
          <div className="flex w-full max-w-2xl items-center">
            <div className="relative flex-1">
              <textarea
                ...
                rows={1}
                maxLength={maxLen}
                className="w-full resize-none rounded-2xl border border-gray-300/70 bg-white/80 px-4 py-3 pr-12 text-sm text-gray-900 placeholder:text-gray-400 focus:border-orange-500 focus:ring-4 focus:ring-orange-500/20 disabled:opacity-50 placeholder:text-sm disabled:cursor-not-allowed transition-all duration-200 outline-none dark:border-zinc-600 dark:bg-zinc-800/70 dark:text-gray-100 dark:placeholder:text-gray-500 dark:focus:border-orange-400"
        ...
        {!user && (
          <p className="mt-2 text-center text-xs text-gray-500 dark:text-gray-400">
            <Link href={loginHref} className="text-orange-600 hover:underline">
              {t("loginCta")}
            </Link>{" "}
            {t("loginFooter")}
          </p>
